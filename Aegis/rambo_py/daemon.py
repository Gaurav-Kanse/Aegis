...
from rambo_py.state import record_event
from rambo_py.notify import send_notification
from rambo_py.utils.proc import total_ram_kb
from rambo_py.score import collect_candidates, rank_candidates
from rambo_py.oom import mark_expendable_processes, run_oom_protect_helper, set_oom_score_adj
from rambo_py.watchers.base import Event, Severity
...
from rambo_py.watchers.battery import BatteryWatcher
...
class Daemon:
    ...
    def run(self):
        print(f"[rambo-py] Daemon starting — soft {self.cfg.memory.soft_pct:.0f}% | hard {self.cfg.memory.hard_pct:.0f}% | max {self.cfg.memory.max_pct:.0f}%")
        
        # Setup signal handling
        signal.signal(signal.SIGINT, self._handle_signal)
        signal.signal(signal.SIGTERM, self._handle_signal)

        # Mark expendables if configured
        if self.cfg.kill.oom_prefer:
            n = mark_expendable_processes(self.cfg.expendable)
            if n > 0:
                print(f"[rambo-py] Marked {n} expendable processes as OOM-preferred")

        # Run OOM protect helper
        ...
        print("[rambo-py] Daemon running. Press Ctrl+C to stop.")
        ...
        print("[rambo-py] Daemon stopped cleanly.")
        ...
        while not self.stop_event.is_set():
            try:
                ...
                if ev.severity in (Severity.CRITICAL, Severity.EMERGENCY) and ev.source in ("memory", "temperature", "pressure"):
                    ...
                    send_notification(f"Rambo Alert ({ev.source.upper()})", ev.message, urgency="normal" if ev.severity == Severity.WARNING else "low")
            except queue.Empty:
                continue
    ...
    def _act_kill(self, ev: Event):
        ...
        if now - self.last_kill_time < cooldown_sec:
            send_notification("Rambo Action", f"Kill throttled (cooldown active): {ev.message}", urgency="high")
            return

        cands = collect_candidates(set(self.cfg.protect))
        if not cands:
            send_notification("Rambo Action", "No kill candidate found", urgency="high")
            return

        ranked = rank_candidates(cands, self.cfg.kill.weights, self.total_ram_bytes, set(self.cfg.expendable))
        victim = ranked[0]

        # Elevate victim oom score
        ...
        try:
            ...
            print(f"[rambo-py] KILL: {msg}")
            send_notification("Rambo Action", msg, urgency="critical")
            record_event("kill", "critical", msg, {"pid": victim.pid, "rss": victim.rss, "score": victim.score})
            self.last_kill_time = now
        except Exception as e:
            print(f"[rambo-py] Failed to kill {victim.name} (PID {victim.pid}): {e}")
